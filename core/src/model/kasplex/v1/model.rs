...
pub mod krc20 {
    ...
    impl Serializer for Op {
        fn serialize<W: std::io::Write>(&self, writer: &mut W) -> std::io::Result<()> {
            store!(u8, &1, writer)?;
            match self {
                Op::Deploy => store!(u8, &0, writer)?,
                Op::Mint => store!(u8, &1, writer)?,
                Op::Transfer => store!(u8, &2, writer)?,
            }
            Ok(())
        }
    }

    impl Deserializer for Op {
    ...
    #[serde_as]
    #[derive(Debug, Deserialize, Serialize, PartialEq, Clone, BorshSerialize, BorshDeserialize)]
    ...
    impl Serializer for TokenTransaction {
        fn serialize<W: std::io::Write>(&self, writer: &mut W) -> std::io::Result<()> {
            store!(u8, &1, writer)?;
            store!(Protocol, &self.protocol, writer)?;
            store!(Op, &self.op, writer)?;
            store!(String, &self.tick, writer)?;
            store!(Option<u128>, &self.max, writer)?;
            store!(Option<u128>, &self.limit, writer)?;
            store!(Option<u64>, &self.decimal, writer)?;
            store!(Option<u128>, &self.amount, writer)?;
            store!(Option<u128>, &self.preallocated, writer)?;
            store!(Option<String>, &self.from, writer)?;
            store!(Option<String>, &self.to, writer)?;
            store!(Option<u64>, &self.op_score, writer)?;
            store!(Option<Hash>, &self.hash_rev, writer)?;
            store!(Option<String>, &self.fee_rev, writer)?;
            store!(Option<String>, &self.tx_accept, writer)?;
            store!(Option<String>, &self.op_accept, writer)?;
            store!(Option<String>, &self.op_error, writer)?;
            store!(Option<String>, &self.mts_add, writer)?;
            store!(Option<String>, &self.mts_mod, writer)?;
            Ok(())
        }
    }

    impl Deserializer for TokenTransaction {
        fn deserialize<R: std::io::Read>(reader: &mut R) -> std::io::Result<Self> {
            let version = load!(u8, reader)?;
            if version != 1 {
                return Err(std::io::Error::new(
            ...
            let protocol = load!(Protocol, reader)?;
            let op = load!(Op, reader)?;
            let tick = load!(String, reader)?;
            let max = load!(Option<u128>, reader)?;
            let limit = load!(Option<u128>, reader)?;
            let dec = load!(Option<u64>, reader)?;
            let amount = load!(Option<u128>, reader)?;
            let pre = load!(Option<u128>, reader)?;
            let from = load!(Option<String>, reader)?;
            let to = load!(Option<String>, reader)?;
            let op_score = load!(Option<u64>, reader)?;
            let hash_rev = load!(Option<Hash>, reader)?;
            let fee_rev = load!(Option<String>, reader)?;
            let tx_accept = load!(Option<String>, reader)?;
            let op_accept = load!(Option<String>, reader)?;
            let op_error = load!(Option<String>, reader)?;
            let mts_add = load!(Option<String>, reader)?;
            let mts_mod = load!(Option<String>, reader)?;
            Ok(TokenTransaction {
                protocol,
                op,
                tick,
