...
use crate::imports::*;
pub use krc721_macros::build_wrpc_client_interface;
...
impl Debug for Inner {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("Krc721RpcClient")
            .field("rpc", &"rpc")
...
#[derive(Clone)]
pub struct Krc721RpcClient {
    inner: Arc<Inner>,
}

impl Debug for Krc721RpcClient {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("Krc721RpcClient")
            .field("url", &self.url())
...
impl Krc721RpcClient {
    pub fn try_new(url: &str, encoding: Option<Encoding>) -> Result<Krc721RpcClient> {
        let inner = Arc::new(Inner::try_new(url, encoding.unwrap_or(Encoding::Borsh))?);
        let client = Krc721RpcClient { inner };
        Ok(client)
    }
    ...
    pub async fn negotiate(&self, network_id: &NetworkId) -> Result<Arc<IndexerStatus>> {
        let GetStatusResponse { response: status } = self.get_status().await?;

        let status_network = NetworkId::from(status.network);
        if status_network != *network_id {
            return Err(Error::NetworkId {
                expected: network_id.to_string(),
                connected: status.network.to_string(),
            });
        }
        Ok(status)
    }
...
impl Krc721RpcClient {
    ...
    pub async fn get_sync_status(&self) -> Result<GetSyncStatusResponse> {
        let request = GetSyncStatusRequest {};
        Ok(self.get_sync_status_call(request).await?)
    }
