...
declare! {
     ...
     */
    export interface IKrc721RpcConfig {
        /**
...
impl Default for Krc721RpcConfig {
    fn default() -> Self {
        RpcConfig { url: None, encoding: Some(Encoding::Borsh), network_id: None, resolver: None }
    }
}

impl TryFrom<IKrc721RpcConfig> for Krc721RpcConfig {
    type Error = Error;
    fn try_from(config: IKrc721RpcConfig) -> Result<Self> {
        ...
        Ok(Krc721RpcConfig { resolver, url, encoding, network_id })
    }
}

impl TryFrom<Krc721RpcConfig> for IKrc721RpcConfig {
    type Error = Error;
    fn try_from(config: Krc721RpcConfig) -> Result<Self> {
        let object = IKrc721RpcConfig::default();
...
impl RpcClient {
    pub fn new(config: Option<Krc721RpcConfig>) -> Result<Krc721RpcClient> {
        let Krc721RpcConfig { resolver, url, encoding, network_id } = config.unwrap_or_default();
...
impl RpcClient {
    ...
    ///
    /// @see {@link IKrc721RpcConfig} interface for more details.
    ///
    #[wasm_bindgen(constructor)]
    pub fn ctor(config: Option<IKrc721RpcConfig>) -> Result<Krc721RpcClient> {
        Self::new(config.map(Krc721RpcConfig::try_from).transpose()?)
    }
