...
- `tokenId` and `price` are read from the inscription JSON
...
- `price` must be present in the inscription
...
- `output[1]` script public key is treated as the buyer destination
...
- `output[0]` payment amount must be greater than or equal to the stored listing price
- `output[1]` must exist and is used as the new owner destination
- On success, ownership transfers to the buyer and the active listing state is removed
